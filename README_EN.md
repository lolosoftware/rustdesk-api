...
| RUSTDESK_API_RUSTDESK_AUTO_ADD_ADDRESS_BOOK_COLLECTION_ID | ID of the named address book receiving devices that report `sysinfo`; `0` disables automatic addition. Entries belong to the collection owner and existing devices are not duplicated. | `0` |
...
| RUSTDESK_API_RUSTDESK_WS_HOST                          | Optional HTTPS/WSS WebSocket proxy base. The client appends `/id` for port 21118 and `/relay` for port 21119                                         | `https://rdapi.example.com/webclient-ws` |
| ---- PROXY -----                                       | ---------------                                                                                                                                     | ----------                    |
